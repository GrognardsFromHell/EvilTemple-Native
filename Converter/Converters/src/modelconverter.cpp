...
public:
    ...
    GetMaterialNameFn getMaterialNameFn;
    GetSkeletonNameFn getSkeletonNameFn;

    QString getMaterialName(const QString &filename);
    QString getSkeletonName(const QString &filename);

    void postprocessMesh(Ogre::Mesh *mesh);

    ModelConverterPrivate(OgreSystems*);
    ~ModelConverterPrivate();

    /* Mesh imports */
    void importMesh(Ogre::Mesh *ogreMesh, const Troika::MeshModel &troikaModel);
    void importGeometry(Ogre::Mesh *ogreMesh, const Troika::MeshModel &troikaModel);
    void importSubmeshes(Ogre::Mesh *ogreMesh, const Troika::MeshModel &troikaModel);
...
ModelConverter::ModelConverter(OgreSystems *ogreSystems) : d(new ModelConverterPrivate(ogreSystems))
{
}
...
ModelConverterPrivate::ModelConverterPrivate(OgreSystems *ogreSystems)
    : ogre(ogreSystems), getMaterialNameFn(0), getSkeletonNameFn(0)
{
}

ModelConverterPrivate::~ModelConverterPrivate()
{
}

QByteArray ModelConverter::convertMesh(const Troika::MeshModel &model)
{
    using namespace Ogre;
    MeshPtr newMesh = MeshManager::getSingleton().createManual("conversion",
                                                               ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME);
    d->importMesh(newMesh.getPointer(), model);

    d->postprocessMesh(newMesh.getPointer());
    ...
    d->ogre->meshSerializer->exportMesh(newMesh.getPointer(), tempFile.fileName().toStdString());

    return tempFile.readAll();
}

QString ModelConverterPrivate::getSkeletonName(const QString &filename)
{
    if (getSkeletonNameFn)
        return getSkeletonNameFn(filename);
    else
        return QString::null;
}

QString ModelConverterPrivate::getMaterialName(const QString &filename)
{
    if (getMaterialNameFn)
        return getMaterialNameFn(filename);
    else
        return QString::null;
}

void ModelConverterPrivate::importMesh(Ogre::Mesh *ogreMesh, const Troika::MeshModel &troikaModel)
{
    using namespace Ogre;
    ...
    importSubmeshes(ogreMesh, troikaModel);

    /* Link to skeleton if there is one */
    if (troikaModel.skeleton()) {
        QString filename = getSkeletonName(troikaModel.skeleton()->filename());
        if (!filename.isEmpty())
...
void ModelConverterPrivate::importGeometry(Ogre::Mesh *ogreMesh, const Troika::MeshModel &troikaModel)
{
    using namespace Ogre;
...
void ModelConverterPrivate::importSubmeshes(Ogre::Mesh *ogreMesh, const Troika::MeshModel &troikaModel)
{
    using namespace Ogre;
    ...
    foreach (const QSharedPointer<Troika::FaceGroup> &faceGroup, troikaModel.faceGroups()) {
        ...
        QString materialName = getMaterialName(faceGroup->material());
...
void ModelConverterPrivate::importBoneAssignments(Ogre::Mesh *ogreMesh, const Troika::MeshModel &troikaModel)
{
...
void ModelConverterPrivate::postprocessMesh(Ogre::Mesh *mesh)
{
    using namespace Ogre;
    ...
    VertexDeclaration* newDcl =
			...
			mesh->hasSkeleton(), mesh->hasVertexAnimation(), mesh->hasVertexAnimation());
...
void ModelConverter::setGetMaterialNameFn(GetMaterialNameFn fn)
{
    d->getMaterialNameFn = fn;
}

void ModelConverter::setGetSkeletonNameFn(GetSkeletonNameFn fn)
{
    d->getSkeletonNameFn = fn;
}
...
QByteArray ModelConverter::convertSkeleton(const Troika::MeshModel &model)
{
    ...
    d->importSkeleton(newSkel.getPointer(), model);
    ...
    try {
        d->ogre->skeletonSerializer->exportSkeleton(newSkel.getPointer(), tempFile.fileName().toStdString());
...
void ModelConverterPrivate::importSkeleton(Ogre::Skeleton *ogreSkeleton, const Troika::MeshModel &troikaModel)
{
...
void ModelConverterPrivate::importBones(Ogre::Skeleton *ogreSkeleton, const Troika::MeshModel &troikaModel)
{
...
void ModelConverterPrivate::importAnimations(Ogre::Skeleton *ogreSkeleton, const Troika::MeshModel &troikaModel)
{
